...
def test_ntfy_posts_with_env_only_settings(monkeypatch) -> None:
    _cfg(
        monkeypatch,
        NTFY_ENABLED=False,
        NTFY_BASE_URL="https://config.invalid",
        NTFY_TOPIC="",
        NTFY_TOKEN="",
        NTFY_PRIORITY="default",
        NTFY_DASHBOARD_URL="",
    ...
    monkeypatch.setenv("NTFY_DASHBOARD_URL", "https://example.com/from-env")
    mock_resp = MagicMock()
    mock_resp.read.return_value = b"ok"
    with patch.object(ntfy_notify, "urlopen", return_value=mock_resp) as mock_open:
        ntfy_notify.maybe_notify_ntfy_qualified_changes(
            [{"symbol": "ETH", "listed_on": ["kraken"]}],
            None,
        )
        mock_open.assert_called_once()
        req = mock_open.call_args[0][0]
        ...
        assert req.headers["Click"] == "https://example.com/from-env"
