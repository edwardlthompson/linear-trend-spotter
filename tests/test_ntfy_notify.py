...
def test_ntfy_posts_from_environment_overrides(monkeypatch) -> None:
    _cfg(
        monkeypatch,
        NTFY_ENABLED=False,
    ...
    monkeypatch.setenv("NTFY_DASHBOARD_URL", "https://example.com/env")
    mock_resp = MagicMock()
    mock_resp.read.return_value = b"ok"
    with patch.object(ntfy_notify, "urlopen", return_value=mock_resp) as mock_open:
        ntfy_notify.maybe_notify_ntfy_qualified_changes([{"symbol": "BTC"}], None)
        ...
        assert req.headers["Click"] == "https://example.com/env"
