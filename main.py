...
def _notify_public_config() -> dict[str, Any] | None:
    return build_notify_public_config(
...
def _finalize_zero_qualified_scan(
    *,
    ...
    """Finalize a successful scan whose filters produced no qualified rows."""
        ...
        try:
            ...
            write_public_qualified_snapshot(
                ...
                scan_health={
                    ...
                    "coins_evaluated": coins_evaluated,
                    "errors_count": _metric_error_total(),
                },
                qualification_exits=exited,
                notify_public_config=_notify_public_config(),
            )
            app_logger.info("📤 Public qualified snapshot written (0 qualified coins)")
    ...
    try:
        ...
        if settings.public_qualified_snapshot_enabled:
            try:
                ...
                notify_public = _notify_public_config()
                write_public_qualified_snapshot(
                    settings.DATA_DIR,
                    settings.public_qualified_snapshot_file,
                    final_results,
