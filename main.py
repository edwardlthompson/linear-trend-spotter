...
def _snapshot_error_count() -> int:
    err_map = metrics.get_summary().get("errors") or {}
    err_total = 0
    for value in err_map.values():
...
def _publish_public_snapshot(
    ...
    *,
    ...
    """Publish the dashboard snapshot, including healthy scans with zero qualified coins."""
    ...
    try:
        ...
        write_public_qualified_snapshot(
            ...
            scan_health={
                ...
                "errors_count": _snapshot_error_count(),
            },
...
def _finalize_zero_qualified_scan(
    *,
    ...
    """Finalize active exits and notifications for a healthy scan with no current qualifiers."""
