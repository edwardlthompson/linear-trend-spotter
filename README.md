...
3. **Qualified dashboard (PWA):** Read-only snapshot with **Qualified** and **Watchlist** tabs (star a symbol on Qualified to track it; Watchlist shows snapshot rows plus off-list starred symbols as “Not in snapshot”), sort, **Health ≥ filter in the Health column header** (spreadsheet-style), search and **multi-exchange** filter, dark/light theme, single **Export** control (format **CSV** or **JSON** for the current tab’s filtered rows), deep links (`#symbol=`), volumes + backtest modal, 7d/30d **1h-close** sparklines (high/low reference lines + last-close marker) when the snapshot includes `closes_1h`, diff banner vs last visit, **Tier-A** timed poll + **service worker** notifications (absolute icon URLs; filtered-table changes and “snapshot refreshed” when JSON changes but filters don’t), **Logs** history persisted in **localStorage**, **Tier-B** Web Push when the scanner **active** list gains/loses a member, relay health strip, optional **API usage / budget** panel when `SCAN_COSTS_ENABLED`, regime strip when the gate blocks qualifications. **Manifest `launch_handler`** prefers a **single desktop client** where the browser supports it. The snapshot **JSON URL** and **Reload** sit under a collapsible **Data source** control so the top bar stays clean; `docs/dashboard/config.js` still sets the default URL.
...
Read-only **PWA** for the hourly qualified snapshot: **Qualified** vs **Watchlist** tabs (star/unstar on the grid), column-header **Health** threshold filter, search, exchange filter, theme, **Export** (CSV or JSON), deep links, optional chart/backtest modal, scan health strip, optional API budget panel, in-browser “update alerts,” and optional **Tier-B Web Push** when you deploy the small relay service. Snapshot URL loading is under **Data source** (collapsed by default).
