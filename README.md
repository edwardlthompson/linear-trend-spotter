...
3. **Qualified dashboard (PWA):** Read-only snapshot with **Qualified** and **Watchlist** tabs (star a symbol on Qualified to track it; Watchlist shows snapshot rows plus off-list starred symbols as “Not in snapshot”), sort, **Health ≥ filter in the Health column header** (spreadsheet-style), search and **multi-exchange** filter, dark/light theme, single **Export** control (format **CSV** or **JSON** for the current tab’s filtered rows), deep links (`#symbol=`), volumes + backtest modal, 1h/daily sparkline when the snapshot includes series, diff banner vs last visit, **Tier-A** timed poll + in-browser notifications scoped to the **filtered Qualified table**, **Tier-B** Web Push when the scanner **active** list gains/loses a member, relay health strip, optional **API usage / budget** panel when `SCAN_COSTS_ENABLED`, regime strip when the gate blocks qualifications. The snapshot **JSON URL** and **Reload** sit under a collapsible **Data source** control so the top bar stays clean; `docs/dashboard/config.js` still sets the default URL.
