...
## Project note (linear-trend-spotter)

Until a dedicated token-sync sprint lands, **`docs/dashboard/styles.css` is the visual source of truth** for the PWA.
[`design-tokens/design-tokens.json`](../design-tokens/design-tokens.json) is present for bootstrap parity; do not rewrite the dashboard to match template `examples/web/` paths. See `docs/BOOTSTRAP_ALIGNMENT.md`.

## Principles

1. **Single token source (template ideal)** — edit colors, spacing, and typography in [`design-tokens/design-tokens.json`](../design-tokens/design-tokens.json), then run `scripts/sync-design-tokens.py` when adopting generated CSS.
2. **Styles and strings are separate** — never put user-visible copy in CSS when an i18n layer exists.
3. **Prefer tokens over raw hex** in new UI — existing dashboard hex values are grandfathered until sync.
