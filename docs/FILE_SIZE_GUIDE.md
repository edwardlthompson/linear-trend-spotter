...
## Project limits (linear-trend-spotter)

This child repo keeps **250 / 150** (not upstream 300 / 150). See `AGENTS.md` and `docs/BOOTSTRAP_ALIGNMENT.md`.

| Tier | Limit | Enforced paths | Purpose |
|------|-------|----------------|---------|
| **View / static** | **250** lines | UI / dashboard / view-oriented files per `scripts/check-file-limits.sh` | Agent-readable views; legacy files may be exempt |
| **Pure logic** | **150** lines | Domain / scanner / backtest logic per `check-file-limits.sh` | Keep domain rules small and testable |
| **Wiring** | Keep thin | Composition roots / entrypoints | Thin composition — no feature logic in the root |

Upstream template documents 300/150 for new greenfield apps; do **not** raise this project's budget without a human decision.
